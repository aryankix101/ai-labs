...
def create_grid(c):
	...
	for i in range(0, 1000, 10):
		for j in range(0, 1000, 10):
			line = c.create_line([(i, j), (i+10, j)], tag='grid_line')
			lines.append(line)
	...
	for i in range(0, 1000, 8):
		for j in range(0, 1000, 10):
			line = c.create_line([(j, i), (j, i+10)], tag='grid_line')
			lines.append(line)
...
canvas = tk.Canvas(root, height=800, width=800, bg='SlateGray1') #creates a canvas widget, which can be used for drawing lines and shapes
create_grid(canvas)
